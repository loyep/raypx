...
export function AuthLayout({
  children,
  logoSlot,
  spotlightClassName = "dark:mix-blend-screen",
  subtitle,
  title,
}: AuthLayoutProps) {
  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-12">
      <div className="absolute inset-0 -z-10 flex items-center justify-center">
        <div
          className={`absolute h-[600px] w-[800px] rounded-full bg-primary/20 blur-[120px] ${spotlightClassName}`}
        />
        <div
          className={`absolute top-1/4 h-[500px] w-[600px] rounded-full bg-cyan-500/10 blur-[120px] ${spotlightClassName}`}
        />
      </div>
